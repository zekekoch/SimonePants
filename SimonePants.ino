...
void musicReactiveFade(byte eq[7]) { //-BOUNCE COLOR (SIMPLE MULTI-LED FADE)
    ...
    byte trailLength = map(bass, 0,255, 3,10);
...
void setup()
{
    ...
    LEDS.addLeds<WS2811,15, GRB>(leds[0], ledCount);
    LEDS.addLeds<WS2811,20, GRB>(leds[1], ledCount);
    LEDS.addLeds<WS2811,17, GRB>(leds[2], ledCount);
...
void loop() {
    ...
    {
        case 0:
            soundPants(0xCC0066, payload.eq);
            break;
        case 1:
...
void soundPants(CRGB color, byte eq[7])
{
    ...
    bass = map(bass, 0, 255, 0, 13);
    int high = (eq[4] + eq[4]) /2;
    high = map(high, 0, 255, 0, 7);
    ...
    for (int i = 14;i>0;i--)
    {
        ...
        {
            leds[0][i] = highlightColor;
            leds[1][i] = highlightColor;
        }
    }
